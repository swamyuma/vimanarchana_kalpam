// ch70: mixed verse blocks — b4,b9 inline-<em>IAST (iast empty); b6,b7,b11,b12,b13 pattern-a
// (separate iast). Unified diff by separating scripts: devWords = words(translit(Deva-only));
// iastWords = words(Latin-only). Works for both since inline blocks carry both scripts in sanskrit.
const { load } = require('./load.js');
const { translit } = require('./translit.js');
const { chapters } = load();
const ch = chapters[71];
if (ch.number !== 72) throw new Error('wrong ch ' + ch.number);
...
console.log('deva कल्ये:', (all.match(/कल्ये/g)||[]).length);
