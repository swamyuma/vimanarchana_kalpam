// Verify: compare patched reader vs backup. Expect changes ONLY in ch35 (idx34) block2.sanskrit;
...
const old = loadFrom('ch35-backup.html');
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch35FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===34){ ch35FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch35:', otherChDiffs);
console.log('ch35 field diffs:', ch35FieldDiffs.join(', ') || '(none)');
// final residual sanity on ch35 b2
const sk=cur[34].blocks[2].sanskrit;
console.log('ch35 b2 residual ou:', sk.split('ou').length-1, ' manṭapa:', sk.split('manṭapa').length-1,
