// Verify patched reader vs ch97-backup.html. Expect: ALL chapters byte-identical EXCEPT ch97 (idx 96);
// block count unchanged (15, colophon b10 pre-existing). Changed fields: b4.sanskrit, b6.iast,
// b7.sanskrit, b7.iast, b9.sanskrit, b10.iast. 0 english diffs; no residual ō/ē/x/ou; fixes present.
const fs = require('fs');
...
const cur = loadFrom(READER), old = loadFrom(SC + '/ch97-backup.html');
if (cur.length !== old.length) throw new Error('chapter count changed');
let outside = 0;
for (let ci = 0; ci < cur.length; ci++) { if (ci === 96) continue;
  if (JSON.stringify(cur[ci]) !== JSON.stringify(old[ci])) { outside++; console.log('UNEXPECTED chapter diff idx', ci); } }
console.log('chapters changed outside ch97:', outside);

const A = cur[96].blocks, O = old[96].blocks;
if (A.length !== 15 || O.length !== 15) throw new Error('block count changed: ' + O.length + ' -> ' + A.length);
const EXPECTED = new Set(['b4.sanskrit', 'b6.iast', 'b7.sanskrit', 'b7.iast', 'b9.sanskrit', 'b10.iast']);
...
console.log('ch97 field diffs:', [...seen].sort().join(', ') || '(none)');
...
if (!A[4].sanskrit.includes('īkṣaṇam')) throw new Error('b4 īkṣaṇam missing');
if (!A[6].iast.includes('tallakṣaṇaṃ') || !A[6].iast.includes('akṣimaṇḍala')) throw new Error('b6 fixes missing');
if (!A[7].iast.includes('akṣimaṇḍalaṃ')) throw new Error('b7 akṣimaṇḍalaṃ missing');
if ((A[7].iast.match(/dvyaṅgulaṃ/g) || []).length !== 6) throw new Error('b7 dvyaṅgulaṃ ×6 missing');
if (!A[7].iast.includes('tryaṅgulaṃ') || A[7].iast.includes('dvyangulaṃ') || A[7].iast.includes('tryangulaṃ')) throw new Error('b7 aṅgula fix incomplete');
if (A[7].sanskrit.includes('ङ्गुुल')) throw new Error('b7 Deva double-u remains');
if ((A[7].sanskrit.match(/ङ्गुल/g) || []).length < 6) throw new Error('b7 Deva ङ्गुल count low');
if (!A[10].colophon || !/pratyāhāravidhirnāma saptanavatitamaḥ paṭalaḥ \|\|97\|\|/.test(A[10].iast)) throw new Error('b10 colophon malformed');
...
if (outside) throw new Error('unexpected diffs outside ch97');
console.log('\nVERIFY OK');
