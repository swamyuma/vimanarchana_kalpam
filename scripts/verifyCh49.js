// Verify: patched reader vs ch49-backup.html. Expect changes ONLY in ch49 (idx48) b2.sanskrit;
...
const old = loadFrom(SC+"/ch49-backup.html");
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===48){ chFieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch49:', otherChDiffs);
console.log('ch49 field diffs:', chFieldDiffs.join(', ') || '(none)');
const sk=cur[48].blocks[2].sanskrit, oldsk=old[48].blocks[2].sanskrit;
const latin = sk.replace(/<[^>]+>/g,' ');
console.log('ch49 b2 residual ou:', (latin.match(/[a-zāīūṛśṣṭḍṇ]ou|ou[a-zāīūṛśṣṭḍṇ]/g)||[]).length,
