// Verify: compare patched reader vs backup. Expect changes ONLY in ch39 (idx35) block2.sanskrit;
...
const old = loadFrom(process.env.SC+"/ch39-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch39FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===38){ ch39FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch39:', otherChDiffs);
console.log('ch39 field diffs:', ch39FieldDiffs.join(', ') || '(none)');
// final residual sanity on ch39 b2
const sk=cur[38].blocks[2].sanskrit;
console.log('ch39 b2 residual ou:', sk.split('ou').length-1, ' manṭapa:', sk.split('manṭapa').length-1,
