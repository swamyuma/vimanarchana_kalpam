// Verify: patched reader vs ch42-backup.html. Expect changes ONLY in ch42 (idx41) block3.sanskrit;
...
const old = loadFrom(SC+"/ch42-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch42FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===41){ ch42FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch42:', otherChDiffs);
console.log('ch42 field diffs:', ch42FieldDiffs.join(', ') || '(none)');
const sk=cur[41].blocks[3].sanskrit, oldsk=old[41].blocks[3].sanskrit;
console.log('ch42 b3 residual ou:', sk.split('ou').length-1,
