// Verify patched reader vs ch88-backup.html. Expect: ALL chapters byte-identical EXCEPT ch88 (idx 87);
// block count unchanged (17, colophon b11 pre-existing); only b4.iast, b5.iast, b6.iast, b8.sanskrit,
// b10.sanskrit, b11.iast changed; 0 english diffs; no residual ō/ē/x/ou in verse sanskrit/iast;
// targeted fixes present.
const fs = require('fs');
...
const cur = loadFrom(READER), old = loadFrom(SC + '/ch88-backup.html');
if (cur.length !== old.length) throw new Error('chapter count changed');
let outside = 0;
for (let ci = 0; ci < cur.length; ci++) { if (ci === 87) continue;
  if (JSON.stringify(cur[ci]) !== JSON.stringify(old[ci])) { outside++; console.log('UNEXPECTED chapter diff idx', ci); } }
console.log('chapters changed outside ch88:', outside);

const A = cur[87].blocks, O = old[87].blocks;
if (A.length !== 17 || O.length !== 17) throw new Error('block count changed: ' + O.length + ' -> ' + A.length);
const EXPECTED = new Set(['b4.iast', 'b5.iast', 'b6.iast', 'b8.sanskrit', 'b10.sanskrit', 'b11.iast']);
...
console.log('ch88 field diffs:', [...seen].sort().join(', ') || '(none)');
...
if (!A[6].iast.includes('retasaḥ bījaṃ')) throw new Error('b6 retasaḥ missing');
if (A[6].iast.includes('retasāṃ')) throw new Error('b6 old retasāṃ remains');
if (!A[8].sanskrit.includes('मासद्वये') || !A[8].sanskrit.includes('māsadvaye śiro')) throw new Error('b8 māsadvaye missing');
if (A[8].sanskrit.includes('माषद्वये') || A[8].sanskrit.includes('māṣadvaye')) throw new Error('b8 old māṣa remains');
if (!A[10].sanskrit.includes('garbhavarṇaḥ (or garbhavikāraḥ)')) throw new Error('b10 garbha fixes missing');
if (A[10].sanskrit.includes('garbhāvarṇaḥ') || A[10].sanskrit.includes('garbhagarbhavarṇaḥ')) throw new Error('b10 old garbha remains');
// colophon b11 intact & normalized
if (!A[11].colophon || !/dehotpattividhirnāma aṣṭāśītitamaḥ paṭalaḥ \|\|88\|\|/.test(A[11].iast)) throw new Error('b11 colophon malformed');
...
if (outside) throw new Error('unexpected diffs outside ch88');
console.log('\nVERIFY OK');
