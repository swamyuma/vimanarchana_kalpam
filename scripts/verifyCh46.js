// Verify: patched reader vs ch46-backup.html. Expect changes ONLY in ch46 (idx45) block3.sanskrit;
...
const old = loadFrom(SC+"/ch46-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch46FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===45){ ch46FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch46:', otherChDiffs);
console.log('ch46 field diffs:', ch46FieldDiffs.join(', ') || '(none)');
const sk=cur[45].blocks[2].sanskrit, oldsk=old[45].blocks[2].sanskrit;
console.log('ch46 b3 residual ou:', sk.split('ou').length-1,
