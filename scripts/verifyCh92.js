// Verify patched reader vs ch92-backup.html. Expect: ALL chapters byte-identical EXCEPT ch92 (idx 91).
// ch92: block count 11 -> 12 (colophon INSERTED at index 7). Mapping: new[0..6]==old[0..6] (with edits
...
const cur = loadFrom(READER), old = loadFrom(SC + '/ch92-backup.html');
if (cur.length !== old.length) throw new Error('chapter count changed');
let outside = 0;
for (let ci = 0; ci < cur.length; ci++) { if (ci === 91) continue;
  if (JSON.stringify(cur[ci]) !== JSON.stringify(old[ci])) { outside++; console.log('UNEXPECTED chapter diff idx', ci); } }
console.log('chapters changed outside ch92:', outside);

const A = cur[91].blocks, O = old[91].blocks;
if (O.length !== 11) throw new Error('backup ch92 not 11 blocks');
if (A.length !== 12) throw new Error(`ch92 expected 12 blocks, got ${A.length}`);

// new[7] colophon
const col = A[7];
if (!col.colophon || col.label !== 'Colophon') throw new Error('b7 not colophon');
if (!/nāḍīnirūpaṇaṃ nāma dvinavatitamaḥ paṭalaḥ \|\|92\|\|/.test(col.iast)) throw new Error('colophon iast malformed');
if (!/नाडीनिरूपणं नाम द्विनवतितमः पटलः ॥९२॥/.test(col.sanskrit)) throw new Error('colophon deva malformed');
if (!/Thus ends the Ninety-Second Chapter/.test(col.english)) throw new Error('colophon english missing');
...
console.log('ch92 head field diffs:', [...seen].sort().join(', ') || '(none)');
...
if (!A[4].sanskrit.includes('nāḍīcakraṃ vakṣye')) throw new Error('b4 vakṣye missing');
if (!A[4].sanskrit.includes('suṣumnaiva')) throw new Error('b4 suṣumnaiva missing');
if (!A[4].sanskrit.includes('आसामादौ') || A[4].sanskrit.includes('आसाम्रादौ')) throw new Error('b4 आसामादौ missing');
if (!A[4].sanskrit.includes('न्तर्गता') || A[4].sanskrit.includes('न्तर्गुता')) throw new Error('b4 अन्तर्गता missing');
if ((A[6].sanskrit.match(/dakṣiṇ/g) || []).length !== 6) throw new Error('b6 dakṣiṇ ×6 missing');
if (A[6].sanskrit.includes('daxiṇ')) throw new Error('b6 old daxiṇ remains');
...
if (outside) throw new Error('unexpected diffs outside ch92');
console.log('\nVERIFY OK');
