// Verify patched reader vs ch75-backup.html. Expect changes ONLY in ch75 (idx 74), in fields
// b7.iast, b9.sanskrit, b19.sanskrit, b21.sanskrit, b23.sanskrit, b23.iast, b24.iast; NO english
// changes anywhere; NO out-of-chapter changes; block count unchanged (30).
const fs = require('fs');
...
const cur = loadFrom(READER), old = loadFrom(SC+'/ch75-backup.html');
if (cur.length !== old.length) throw new Error('chapter count changed');

let outside = 0, ch75 = [], engChanged = 0;
  ...
  for (let bi=0; bi<A.length; bi++){
    for (const k of new Set([...Object.keys(A[bi]),...Object.keys(B[bi])])){
      if (JSON.stringify(A[bi][k]) !== JSON.stringify(B[bi][k])){
        ...
        if (ci===74) ch75.push(`b${bi}.${k}`);
        else { outside++; console.log('UNEXPECTED diff ch',ci,'b',bi,'field',k); }
      }
    }
  }
}
console.log('diffs outside ch75:', outside);
console.log('english diffs (must be 0):', engChanged);
console.log('ch75 field diffs:', ch75.sort().join(', '));

const expected = 'b19.sanskrit, b21.sanskrit, b23.iast, b23.sanskrit, b24.iast, b7.iast, b9.sanskrit';
// markup balance on touched blocks
for (const bi of [7,9,19,21,23,24]) for (const f of ['sanskrit','iast']) {
  const v = cur[74].blocks[bi][f] || '';
...
if (outside) throw new Error('unexpected diffs outside ch75');
if (engChanged) throw new Error('english field changed');
if (ch75.sort().join(', ') !== expected) throw new Error('ch75 diff set unexpected: '+ch75.sort().join(', '));
console.log('\nVERIFY OK');
