// Verify: compare patched reader vs backup. Expect changes ONLY in ch37 (idx35) block2.sanskrit;
...
const old = loadFrom(process.env.SC+"/ch37-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch37FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===36){ ch37FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch37:', otherChDiffs);
console.log('ch37 field diffs:', ch37FieldDiffs.join(', ') || '(none)');
// final residual sanity on ch37 b2
const sk=cur[36].blocks[2].sanskrit;
console.log('ch37 b2 residual ou:', sk.split('ou').length-1, ' manṭapa:', sk.split('manṭapa').length-1,
