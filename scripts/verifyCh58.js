// Verify: patched reader vs ch58-backup.html. Expect changes ONLY in ch58 (idx57):
...
const old = loadFrom(SC+"/ch58-backup.html");
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===57){ chFieldDiffs.push(`b${bi}.${k}`); }
        else { otherChDiffs++; console.log('UNEXPECTED diff ch idx',ci,'b',bi,'field',k); }
      }
    }
  }
}
console.log('english diffs (any chapter):', engDiffs, '(expect 1: ch58 b3 colophon fill)');
console.log('diffs outside ch58:', otherChDiffs);
console.log('ch58 field diffs:', chFieldDiffs.join(', ') || '(none)');
const sk=cur[57].blocks[2].sanskrit;
const latin = sk.replace(/<[^>]+>/g,' ');
console.log('ch58 b2 residual ou:', (latin.match(/[a-zāīūṛśṣṭḍṇ]ou|ou[a-zāīūṛśṣṭḍṇ]/g)||[]).length,
  ' residual x:', (sk.match(/x/g)||[]).length,
  ' PB:', (sk.match(/<!--PB-->/g)||[]).length,
  ' em bal:', (sk.match(/<em>/g)||[]).length, '/', (sk.match(/<\/em>/g)||[]).length);
console.log('b3.english filled:', cur[57].blocks[3].english.length>0, ' old empty:', old[57].blocks[3].english==='');
