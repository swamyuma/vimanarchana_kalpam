// Verify patched reader vs ch95-backup.html. Expect: ALL chapters byte-identical EXCEPT ch95 (idx 94);
// block count unchanged (12, colophon b7 pre-existing). Changed fields: b4.sanskrit, b6.sanskrit,
// b7.iast. 0 english diffs; no residual ō/ē/x/ou; fix present.
const fs = require('fs');
...
const cur = loadFrom(READER), old = loadFrom(SC + '/ch95-backup.html');
if (cur.length !== old.length) throw new Error('chapter count changed');
let outside = 0;
for (let ci = 0; ci < cur.length; ci++) { if (ci === 94) continue;
  if (JSON.stringify(cur[ci]) !== JSON.stringify(old[ci])) { outside++; console.log('UNEXPECTED chapter diff idx', ci); } }
console.log('chapters changed outside ch95:', outside);

const A = cur[94].blocks, O = old[94].blocks;
...
console.log('ch95 field diffs:', [...seen].sort().join(', ') || '(none)');
...
if (!A[4].sanskrit.includes('aṣṭākṣaraṃ') || A[4].sanskrit.includes('aṣṭāxaraṃ')) throw new Error('b4 aṣṭākṣaraṃ missing');
if (!A[7].colophon || !/jñānasvarūpanirūpaṇaṃ nāma pañcanavatitamaḥ paṭalaḥ \|\|95\|\|/.test(A[7].iast)) throw new Error('b7 colophon malformed');
// spot-check a couple ō/ē normalizations in b6
if (!A[6].sanskrit.includes('pañcadhā pañcātmā') || !A[6].sanskrit.includes('catasro mūrtayaḥ')) throw new Error('b6 norm check failed');
...
console.log('markup balanced; fix present; no residuals.');
if (outside) throw new Error('unexpected diffs outside ch95');
console.log('\nVERIFY OK');
