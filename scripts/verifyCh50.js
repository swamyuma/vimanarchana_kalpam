// Verify: patched reader vs ch50-backup.html. Expect changes ONLY in ch50 (idx49) b2.sanskrit;
// english byte-identical everywhere (0 diffs); no other chapter/field touched; PB intact.
const fs = require('fs');
...
const old = loadFrom(SC+"/ch50-backup.html");
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===49){ chFieldDiffs.push(`b${bi}.${k}`); }
        else { otherChDiffs++; console.log('UNEXPECTED diff ch idx',ci,'b',bi,'field',k); }
      }
    }
  }
}
console.log('english diffs (any chapter):', engDiffs, '(expect 0)');
console.log('diffs outside ch50:', otherChDiffs);
console.log('ch50 field diffs:', chFieldDiffs.join(', ') || '(none)');
const sk=cur[49].blocks[2].sanskrit, oldsk=old[49].blocks[2].sanskrit;
const latin = sk.replace(/<[^>]+>/g,' ');
console.log('ch50 b2 residual ou:', (latin.match(/[a-zāīūṛśṣṭḍṇ]ou|ou[a-zāīūṛśṣṭḍṇ]/g)||[]).length,
  ' residual x:', (sk.match(/x/g)||[]).length,
  ' residual latin+deva mix:', (sk.match(/[a-zāīūṛṃḥśṣṭḍṇñṅ]+[ऀ-ॿ]|[ऀ-ॿ]+[a-zāīūṛṃḥśṣṭḍṇñṅ]/g)||[]).length,
  ' PB old/new:', (oldsk.split('<!--PB-->').length-1)+'/'+(sk.split('<!--PB-->').length-1),
  ' em bal:', (sk.match(/<em>/g)||[]).length, '/', (sk.match(/<\/em>/g)||[]).length);
if(otherChDiffs || engDiffs!==0 || chFieldDiffs.join(',')!=='b2.sanskrit') throw new Error('verification FAILED');
