// Verify: patched reader vs ch44-backup.html. Expect changes ONLY in ch44 (idx43) block3.sanskrit;
...
const old = loadFrom(SC+"/ch44-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch44FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===43){ ch44FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch44:', otherChDiffs);
console.log('ch44 field diffs:', ch44FieldDiffs.join(', ') || '(none)');
const sk=cur[43].blocks[2].sanskrit, oldsk=old[43].blocks[2].sanskrit;
console.log('ch44 b3 residual ou:', sk.split('ou').length-1,
