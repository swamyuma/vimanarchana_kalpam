// Verify: patched reader vs ch67-backup.html. Expect changes ONLY in ch67 (idx66):
...
const old = loadFrom(SC+"/ch67-backup.html");
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===66){ chFieldDiffs.push(`b${bi}.${k}`); }
        else { otherChDiffs++; console.log('UNEXPECTED diff ch idx',ci,'b',bi,'field',k); }
      }
    }
  }
}
console.log('english diffs (any chapter):', engDiffs, '(expect 1: ch67 b3 colophon fill)');
console.log('diffs outside ch67:', otherChDiffs);
console.log('ch67 field diffs:', chFieldDiffs.join(', ') || '(none)');
const sk=cur[66].blocks[2].sanskrit;
const latin = sk.replace(/<[^>]+>/g,' ');
console.log('ch67 b2 residual ou:', (latin.match(/[a-zāīūṛśṣṭḍṇ]ou|ou[a-zāīūṛśṣṭḍṇ]/g)||[]).length,
  ' residual x:', (sk.match(/x/g)||[]).length,
  ' PB:', (sk.match(/<!--PB-->/g)||[]).length,
  ' em bal:', (sk.match(/<em>/g)||[]).length, '/', (sk.match(/<\/em>/g)||[]).length);
console.log('b3.english filled:', cur[66].blocks[3].english.length>0, ' old empty:', old[66].blocks[3].english==='');
