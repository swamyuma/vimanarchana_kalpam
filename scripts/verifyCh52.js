// Verify: patched reader vs ch52-backup.html. Expect changes ONLY in ch52 (idx51) b2.sanskrit;
...
const old = loadFrom(SC+"/ch52-backup.html");
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===51){ chFieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch52:', otherChDiffs);
console.log('ch52 field diffs:', chFieldDiffs.join(', ') || '(none)');
const sk=cur[51].blocks[2].sanskrit, oldsk=old[51].blocks[2].sanskrit;
const latin = sk.replace(/<[^>]+>/g,' ');
console.log('ch52 b2 residual ou:', (latin.match(/[a-zāīūṛśṣṭḍṇ]ou|ou[a-zāīūṛśṣṭḍṇ]/g)||[]).length,
  ' residual x:', (sk.match(/x/g)||[]).length,
  ' residual muṣ/havis:', (sk.match(/muṣ|havis/g)||[]).length,
