// Verify: patched reader vs ch45-backup.html. Expect changes ONLY in ch45 (idx44) block3.sanskrit;
...
const old = loadFrom(SC+"/ch45-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch45FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===44){ ch45FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch45:', otherChDiffs);
console.log('ch45 field diffs:', ch45FieldDiffs.join(', ') || '(none)');
const sk=cur[44].blocks[2].sanskrit, oldsk=old[44].blocks[2].sanskrit;
console.log('ch45 b3 residual ou:', sk.split('ou').length-1,
