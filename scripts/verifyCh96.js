// Verify patched reader vs ch96-backup.html. Expect: ALL chapters byte-identical EXCEPT ch96 (idx 95);
// block count unchanged (15, colophon b10 pre-existing). Changed fields: b4.sanskrit, b6.iast, b7.iast,
// b9.sanskrit, b10.sanskrit, b10.iast. 0 english diffs; no residual ō/ē/x/ou; fixes present.
const fs = require('fs');
...
const cur = loadFrom(READER), old = loadFrom(SC + '/ch96-backup.html');
if (cur.length !== old.length) throw new Error('chapter count changed');
let outside = 0;
for (let ci = 0; ci < cur.length; ci++) { if (ci === 95) continue;
  if (JSON.stringify(cur[ci]) !== JSON.stringify(old[ci])) { outside++; console.log('UNEXPECTED chapter diff idx', ci); } }
console.log('chapters changed outside ch96:', outside);

const A = cur[95].blocks, O = old[95].blocks;
if (A.length !== 15 || O.length !== 15) throw new Error('block count changed: ' + O.length + ' -> ' + A.length);
const EXPECTED = new Set(['b4.sanskrit', 'b6.iast', 'b7.iast', 'b9.sanskrit', 'b10.sanskrit', 'b10.iast']);
...
console.log('ch96 field diffs:', [...seen].sort().join(', ') || '(none)');
...
if (!A[4].sanskrit.includes('vakṣye') || (A[4].sanskrit.match(/lakṣaṇam/g) || []).length !== 2) throw new Error('b4 fixes missing');
for (const t of ['dakṣiṇapādam', 'gomukhaṃ', 'gomukham', 'prekṣaṇam', 'nikṣipya', 'nirīkṣya', 'vāmapāṇiṃ', 'ūrvorupari', 'pādau daṇḍavad', 'karatale'])
  if (!A[6].iast.includes(t)) throw new Error('b6 missing ' + t);
if (A[6].iast.includes('daxiṇ') || A[6].iast.includes('mux') || A[6].iast.includes('exaṇ')) throw new Error('b6 residual x-token');
if (!A[7].iast.includes('gomukha-siṃha')) throw new Error('b7 gomukha missing');
for (const t of ['kuṇḍalinīmukhaṃ', 'ūrdhvamukhaṃ', 'tatrasthaṃ puruṣaṃ ca nābhau', 'yāvacchakyaṃ', 'niḥśvāsanirodhaḥ'])
  if (!A[9].sanskrit.includes(t)) throw new Error('b9 missing ' + t);
if (!A[10].sanskrit.includes('श्रीवैखानसे') || A[10].sanskrit.includes('वैखांनसे')) throw new Error('b10 vaikhānase fix missing');
if (!A[10].sanskrit.includes('विधिर्नाम') || A[10].sanskrit.includes('नांम')) throw new Error('b10 nāma fix missing');
if (!A[10].colophon) throw new Error('b10 not colophon');
...
if (outside) throw new Error('unexpected diffs outside ch96');
console.log('\nVERIFY OK');
