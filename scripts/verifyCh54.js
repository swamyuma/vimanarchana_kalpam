// Verify: patched reader vs ch54-backup.html. Expect changes ONLY in ch54 (idx53):
...
const old = loadFrom(SC+"/ch54-backup.html");
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===53){ chFieldDiffs.push(`b${bi}.${k}`); }
        else { otherChDiffs++; console.log('UNEXPECTED diff ch idx',ci,'b',bi,'field',k); }
      }
    }
  }
}
console.log('english diffs (any chapter):', engDiffs, '(expect 1: ch54 b3 colophon fill)');
console.log('diffs outside ch54:', otherChDiffs);
console.log('ch54 field diffs:', chFieldDiffs.join(', ') || '(none)');
const sk=cur[53].blocks[2].sanskrit, oldsk=old[53].blocks[2].sanskrit;
const latin = sk.replace(/<[^>]+>/g,' ');
console.log('ch54 b2 residual ou:', (latin.match(/[a-zāīūṛśṣṭḍṇ]ou|ou[a-zāīūṛśṣṭḍṇ]/g)||[]).length,
  ' residual x:', (sk.match(/x/g)||[]).length,
  ' residual muṣ(non-ṭ)/havis:', (sk.match(/muṣ(?!ṭ)|havis/g)||[]).length,
  ' PB old/new:', (oldsk.split('<!--PB-->').length-1)+'/'+(sk.split('<!--PB-->').length-1),
  ' em bal:', (sk.match(/<em>/g)||[]).length, '/', (sk.match(/<\/em>/g)||[]).length);
console.log('b3.english filled:', cur[53].blocks[3].english.length>0, ' old empty:', old[53].blocks[3].english==='');
