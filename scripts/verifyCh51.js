// Verify: patched reader vs ch51-backup.html. Expect changes ONLY in ch51 (idx50) b2.sanskrit;
...
const old = loadFrom(SC+"/ch51-backup.html");
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===50){ chFieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch51:', otherChDiffs);
console.log('ch51 field diffs:', chFieldDiffs.join(', ') || '(none)');
const sk=cur[50].blocks[2].sanskrit, oldsk=old[50].blocks[2].sanskrit;
const latin = sk.replace(/<[^>]+>/g,' ');
console.log('ch51 b2 residual ou:', (latin.match(/[a-zāīūṛśṣṭḍṇ]ou|ou[a-zāīūṛśṣṭḍṇ]/g)||[]).length,
  ' residual x:', (sk.match(/x/g)||[]).length,
  ' residual muṣ/praṣ:', (sk.match(/muṣ|praṣ/g)||[]).length,
