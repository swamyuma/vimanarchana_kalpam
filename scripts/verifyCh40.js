// Verify: patched reader vs ch40-backup.html. Expect changes ONLY in ch40 (idx39) block2.sanskrit;
// english byte-identical everywhere; no other chapter/field touched.
const fs = require('fs');
const READER = require('./load.js').READER;
const SC = process.env.SC || 'C:/Users/umasu/AppData/Local/Temp/claude/C--Users-umasu-Documents-gate-files-vimanarchana-reader/e28aea44-7500-4af2-9eac-df0010e61114/scratchpad';
function loadFrom(p){
...
const old = loadFrom(SC+"/ch40-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch40FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===39){ ch40FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch40:', otherChDiffs);
console.log('ch40 field diffs:', ch40FieldDiffs.join(', ') || '(none)');
const sk=cur[39].blocks[2].sanskrit;
console.log('ch40 b2 residual ou:', sk.split('ou').length-1,
