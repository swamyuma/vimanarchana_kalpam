// Verify: compare patched reader vs backup. Expect changes ONLY in ch36 (idx35) block2.sanskrit;
...
const old = loadFrom('ch36-backup.html');
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch36FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===35){ ch36FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch36:', otherChDiffs);
console.log('ch36 field diffs:', ch36FieldDiffs.join(', ') || '(none)');
// final residual sanity on ch36 b2
const sk=cur[35].blocks[2].sanskrit;
console.log('ch36 b2 residual ou:', sk.split('ou').length-1, ' manṭapa:', sk.split('manṭapa').length-1,
