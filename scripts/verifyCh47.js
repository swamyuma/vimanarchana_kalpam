// Verify: patched reader vs ch47-backup.html. Expect changes ONLY in ch47 (idx46) block3.sanskrit;
...
const old = loadFrom(SC+"/ch47-backup.html");
if(cur.length!==old.length) throw new Error('chapter count changed');
let engDiffs=0, otherChDiffs=0, ch47FieldDiffs=[];
  ...
  for(let bi=0;bi<ba.length;bi++){
    for(const k of new Set([...Object.keys(ba[bi]),...Object.keys(bb[bi])])){
      if(JSON.stringify(ba[bi][k])!==JSON.stringify(bb[bi][k])){
        ...
        if(ci===46){ ch47FieldDiffs.push(`b${bi}.${k}`); }
...
console.log('diffs outside ch47:', otherChDiffs);
console.log('ch47 field diffs:', ch47FieldDiffs.join(', ') || '(none)');
const sk=cur[46].blocks[2].sanskrit, oldsk=old[46].blocks[2].sanskrit;
console.log('ch47 b3 residual ou:', sk.split('ou').length-1,
